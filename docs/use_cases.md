...
- **Flow**:
    ...
    2.  Selects the medicine from the master list.
...
- **Flow**:
    ...
    2.  The dashboard defaults to a company-wide view, showing total revenue, profit, and top-selling medicines across all stores.
...
- **Flow**:
    1.  Company Stockist identifies that "Store A" has 200 units of a medicine, while "Store B" has only 5.
    2.  They initiate a "New Store Transfer".
    3.  They select the medicine, the "From Store" (Store A), and the "To Store" (Store B).
...
### UC-07: Store Manager Checks for Expiring Medicines
